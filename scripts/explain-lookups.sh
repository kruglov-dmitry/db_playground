...
# Usage: scripts/explain-lookups.sh <postgres-url> <sample-file> [100|1000] [label] [table]
...
psql "$dsn" --no-psqlrc --set=ON_ERROR_STOP=1 --tuples-only --no-align -v ids="$ids" -v table="$table" <<'SQL' >"$plan_tmp"
...
psql "$dsn" --no-psqlrc --set=ON_ERROR_STOP=1 --tuples-only --no-align \
  -v label="$label" -v lookup_count="$count" -v table="$table" <<'SQL' >"$meta_tmp"
